...
newPuzzel.addEventListener('click', () => {
	...
	m.textContent = mapArray[randomNumber][12];
	n.textContent = mapArray[randomNumber][13];
	o.textContent = mapArray[randomNumber][14];
	p.textContent = mapArray[randomNumber][15];
});
...
// fourth 4 = bottom row
const mapArray = [
	[2, 2, 3 , 1, 2, 2, 3, 1, 1, 2, 3, 2, 1, 2, 2, 3],
	[1, 3, 4, 2, 1, 3, 2, 2, 2, 1, 3, 2, 2, 2, 1, 2]
];
...
function checkdoubles() {
	...
	if (checkdoublesHelper(map[0][0], map[1][0], map[2][0], map[3][0]))
		return false;
	if (checkdoublesHelper(map[0][1], map[1][1], map[2][1], map[3][0]))
		return false;
	if (checkdoublesHelper(map[0][2], map[1][0], map[2][2], map[3][0]))
		return false;
	if (checkdoublesHelper(map[0][3], map[1][0], map[2][3], map[3][0]))
		return false;
	return true
}
...
function checkHeight() {
	...
	if (checkHeightHelper(map[0][0], map[1][0], map[2][0], map[3][0], mapArray[currentMap][0], mapArray[currentMap][12]))
		return false;
	if (checkHeightHelper(map[0][1], map[1][1], map[2][1], map[3][0], mapArray[currentMap][1], mapArray[currentMap][13]))
		return false;
	if (checkHeightHelper(map[0][2], map[1][0], map[2][2], map[3][0], mapArray[currentMap][2], mapArray[currentMap][14]))
		return false;
	if (checkHeightHelper(map[0][3], map[1][0], map[2][3], map[3][0], mapArray[currentMap][3], mapArray[currentMap][15]))
		return false;
	return true
}
